...
### Files
- `main.rs` - Binary entrypoint; orchestrates ingest/follow modes, connects modules, and manages shutdown.
  - **Key items**: `main()`, `IngestProgress`, `wait_for_peer_head()`, `wait_for_min_peers()`
...
- **Modules**: `cli`, `logging`, `p2p`, `rpc`, `storage`, `sync`, `ui` - primary subsystems wired together by `main.rs`.
